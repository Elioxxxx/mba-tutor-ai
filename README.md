# 电子科技大学 MBA AI 导师匹配系统
**产品需求文档 (PRD) 暨 产品白皮书**

---

## 编者按
本项目旨在彻底解决 MBA 学生在“论文导师双选”环节中面临的“信息差大、自身定位模糊、盲目选择导致后期痛苦”的难题。通过引入大语言模型（MiniMax）和深度的导师画像标签库，为每一位 MBA 学生提供个性化、咨询级别的“诊断+匹配”服务。
...
```mermaid
sequenceDiagram
    participant U as MBA 学生
    participant F as 前端 (React)
    participant API as 后端 API (Express)
    participant M as 核心 AI (MiniMax)
    participant DB as 数据库 (Supabase)

    U->>F: 填写基本表单 & 业务痛点，上传 PDF 简历
    F->>API: 提交数据 /api/analyze
    API->>M: 简历解析与学生画像构建
    M-->>API: 返回结构化学生画像与缺失信息
    API-->>F: 展示“学生洞察摘要”，提示待补充项

    U->>F: (可选)文字补充履历漏洞
    F->>API: /api/analyze/supplement
    API->>M: 完善该学生的背景理解

    U->>F: 从下拉框选取“心仪导师”（可选）
    U->>F: 确认信息，发起匹配请求
    F->>API: /api/match
    API->>DB: 提取全库 95 名有效导师(带课题)的标签
    API->>M: 输入学生画像 + 95位导师信息 + “心仪导师”名单
    M-->>API: 生成 Top 5 榜单与心仪导师专属测评
    API->>DB: 持久化存储结果
    API-->>F: 返回 AI 诊断与推荐报表
    F-->>U: 沉浸式展示匹配理由与具体选题建议
```
...
**目标：提供咨询公司级别的 Matching Report**
大模型进行并行计算，遵循极为严格的优先级打分逻辑：
...
- **心仪导师专区双流架构**：
  - 如果心仪导师**登榜 Top 5**：卡片悬挂 💖 **“用户意向”** 标签，并注入专属评估意见，证明双向奔赴。
  - 如果心仪导师**落榜 Top 5**：在推荐榜下方开辟独立**【未入围导师测评区】**。AI 会非常理智地指出不匹配的风险点（如：“该导师课题纯宏观政策，无法解决你聚焦的 B 端销售转化率问题，存在难以沟通的风险”）。
...
## 五、 未来演进规划 (Roadmap)
1. **防护与风控 (Phase 2)**：
   - 引入 API 节流控制 (express-rate-limit)。
   - 内容合规检测与防护性提示拦截机制。
2. **导师反向挑选仪表盘 (Phase 3)**：
   - 为学院和导师开发看盘系统，查阅有哪些“高匹配度”学生向自己推荐了。
3. **社交与学长学姐点评关联 (Phase 4)**：
   - 引入历年学生对导师的私密评星词云，纳入大语言模型的评估因素。
